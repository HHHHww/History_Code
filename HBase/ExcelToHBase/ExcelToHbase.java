...
public class ExcelToHbase {
	...
	public static void FromHBase() {
		...
		int count = 1;
		String colfam = "sinfo";
		String col[] = { "sname", "sage", "ssex", "smath", "scomputer", "senglish" };
		int j = 0;
		for (int i = 2; i < 32; i++) {
			for (int o = 0; o < 6; o++) {
				rowkey = 9999 + i;
				String rowkey1 = rowkey + "";
				...
				try {
					getData("ExcelToHBase", rowkey1, colfam, col[j]);
	...
	public static void ETHBase() throws Exception, IOException {
		...
		createTable("ExcelToHBase", new String[] { "sinfo", "sscores" });
		int rowkey = 0;
		int count = 1;
		String colfam = "sinfo";
		String col[] = { "sname", "sage", "ssex", "smath", "scomputer", "senglish" };
		for (int i = 2; i < 32; i++) {
			...
			for (Cell cell : row) {
				...
				if (!getCellValueByCell(cell).equals("")) {
					insertData("ExcelToHBase", rowkey1, colfam, col[j], getCellValueByCell(cell));
				}
				j++;
				if (j == 6) {
					j = 0;
